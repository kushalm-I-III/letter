...
export type GameProgressApi = {
  isLoading: boolean;
  error: unknown;
  bySection: Map<GameSection, ProgressRow>;
  completedSections: Set<GameSection>;
  ensureStarted: (section: GameSection) => Promise<void>;
  setCheckpoints: (section: GameSection, checkpointCount: number) => Promise<void>;
  markCheckpoint: (section: GameSection) => Promise<void>;
  markComplete: (section: GameSection) => Promise<void>;
};

const GameProgressContext = createContext<GameProgressApi | null>(null);
...
export function useProgress(): GameProgressApi {
  const ctx = useContext(GameProgressContext);
  if (!ctx) {
    throw new Error("useProgress must be used within GameProgressProvider");
  }
  return ctx;
}
